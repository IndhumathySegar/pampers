...
var KTUtil = (function () {
  ...
  return {
    ...
    isInResponsiveRange: function (mode) {
      ...
      if (
        mode == "general" ||
        (mode == "desktop" && breakpoint >= this.getBreakpoint("lg") + 1) ||
        (mode == "tablet" &&
          breakpoint >= this.getBreakpoint("md") + 1 &&
          breakpoint < this.getBreakpoint("lg")) ||
        (mode == "mobile" && breakpoint <= this.getBreakpoint("md")) ||
    ...
    getUniqueID: function (prefix) {
      return prefix + Math.floor(rn() * new Date().getTime());
    },
    ...
    getByTag: function (query) {
      var el = document.getElementsByTagName(query);
    ...
    getByClass: function (query) {
      var el = document.getElementsByClassName(query);
    ...
    hasClasses: function (el, classes) {
      ...
      for (var i in classesArr) {
        if (!KTUtil.hasClass(el, KTUtil.trim(classesArr[i]))) {
          return false;
        }
      }
    ...
    triggerEvent: function (node, eventName) {
      ...
      if (node.dispatchEvent) {
        ...
        // IE-old school style
        var eventV = doc.createEventObject();
        eventV.synthetic = true; // allow detection of synthetic events
        node.fireEvent("on" + eventName, eventV);
      }
    },
    ...
    data: function (element) {
      ...
      return {
        ...
        get: function (name) {
          this.getData(element, name, data);
        },

        has: function (name) {

          if (!element || !element.customDataTag) {
            return false;
          }
    ...
    getScroll: function (element, method) {
      ...
      const argMethod = method == "scrollTop" ? "pageYOffset" : "pageXOffset";
        ...
        if (defaultView && defaultView.getComputedStyle) {
          ...
          if (/^\d+(em|pt|%|ex)?$/i.test(value)) {
            return (function (value1) {
              ...
              el.style.left = value1 || 0;
              value1 = el.style.pixelLeft + "px";
    ...
    scrollInit: function (element, options) {
      ...
      function init() {
        var result = KTUtil.scrollContent(options, element);
    ...
    scrollContent: function (options, element) {
      var ps;
      var height;
      ...
      var result = KTUtil.scrollValue(options, ps, height, element);
      ...
      return { options: options, element: element };
    },
    scrollValue: function (options, ps, height, element) {
      ...
      return KTUtil.scrollNew(element, height, options);
    },
    scrollNew: function (element, height, options) {
      if (height > 0) {
        KTUtil.css(element, "height", height + "px");
      }
