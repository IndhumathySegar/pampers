...
export class MenuHorizontalComponent implements OnInit {
  ...
  isMenuRootItemIsActive(item): boolean {
    ...
    if (item.submenu.items) {
      return this.checkMenuItem(item.submenu.items);
    }

    if (item.submenu.columns) {
      return this.checkMenuItem(item.submenu.columns);
    }

    if (typeof item.submenu[Symbol.iterator] === "function") {
      return this.checkMenuItem(item.submenu);
    }
   ...
   * @param menuItems: any
   */
  checkMenuItem(menuItems): boolean {
